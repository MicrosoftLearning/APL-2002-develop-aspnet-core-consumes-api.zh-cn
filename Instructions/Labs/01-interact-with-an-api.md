...
1. 查看操作的各个部分，并注意下表中显示的信息。

    | 部分 | 说明 |
    |---|--|
    | **终结点** | 显示在操作标题中。 终结点显示为 `/fruitlist`。 完整 URI 是 API 的基本 URL，在我们的示例中附加了指定的终结点`http://localhost:5050/fruitlist`。 |
    | **Parameters** | 此操作不需要。 |
    | **媒体类型** | 指定操作将返回的媒体类型编码。 |
...
1. 查看操作的各个部分，并注意下表中显示的信息。
    ...
    | 部分 | 说明 |
    |---|--|
    | **终结点** | 终结点显示为 `/fruitlist`。 完整 URI 是 API 的基本 URL，在我们的示例中附加了指定的终结点`http://localhost:5050/fruitlist`。 |
    | **Parameters** | 此操作不需要。 |
...
1. 运行**获取列表中的所有水果**部分中的`GET`命令，注意现在包含了 *Pear* 的记录。

### 任务 3：执行`DELETE`操作

1. 单击 **DELETE** 操作框中的任意位置，展开按 **Id 删除水果**部分中的 **DELETE** 操作。

1. 查看操作的各个部分，并注意下表中显示的信息。

    | 部分 | 说明 |
    |---|--|
    | **终结点** | 终结点显示为 `/fruitlist/{id}`。 完整 URI 是 API 的基本 URL，并附加了指定的`id`删除内容。 例如，`http://localhost:5050/fruitlist/1`指向`id`等于`1`。
...
* 使用 Azure 应用服务扩展将 API 发布到应用程序服务
...
1. 登录完成后，扩展将显示帐户中可用订阅的列表。 下面的截图就是一个例子。
